            ...
            fs_1.default.createReadStream(req.file.path)
                ...
                for (const row of results) {
                    ...
                    if (row.track_id) {
                        ...
                        const track = database_1.default.prepare('SELECT id FROM tracks WHERE title = ?').get(row.track_title);
                        if (track)
                            trackId = track.id;
                    }
                    if (!trackId) {
...
const getMyWithholdings = async (req, res) => {
    ...
    `).all(req.user.id);
        ...
        const withholdings = database_1.default.prepare(`SELECT * FROM royalty_withholdings WHERE track_id IN (${placeholders})`).all(...trackIds);
        res.json(withholdings);
    }
    catch (error) {
        console.error(error);
...
const releaseWithholding = (req, res) => {
    ...
    try {
        ...
        database_1.default.prepare('UPDATE royalty_withholdings SET estado = "liberado", released_at = ? WHERE id = ?').run(new Date().toISOString(), withholdingId);
